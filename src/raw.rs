...
/// to be an error.
pub fn decompress_buf(input: &[u8], output: &mut &mut [u8]) -> Result<usize, Error> {
...
mod tests {
    ...
    fn compress_smoke() {
...

        {
            ...
            assert_eq!(
                c.compress(CompressOp::Finish, input, &mut data),
                Ok(CoStatus::Finished)
            );
...

        {
            ...
            let input = &mut &b"hel"[..];
            assert_eq!(
                c.compress(CompressOp::Flush, input, &mut data),
                Ok(CoStatus::Finished)
            );
            assert!(input.is_empty());
            let input = &mut &b"lo!"[..];
